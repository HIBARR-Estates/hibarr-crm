{{--
  Hibarr branded Task reminder (App\Models\Task) — SMTP / UNS body fallback.
  Keep in sync with resources/views/mail/task/task-reminder.plunk.html
  Note: mail.task.reminder is the legacy markdown template used by older task notifications.
--}}
...
<span style="display:inline-block;width:18px;height:2px;background-color:#003160;vertical-align:middle;margin-right:10px"></span>Task Reminder
</td>
</tr>
</table>
</td>
...
{{ $dueDate }}@if($dueTime)&ensp;&middot;&ensp;{{ $dueTime }}@endif
</div>
<div style="font-family:'DM Mono',Consolas,monospace;font-size:14px;color:#4a5272;line-height:1.6;margin-bottom:4px">
{{ $taskHeading }}@if($taskShortCode) <span style="color:#8a94b0">#{{ $taskShortCode }}</span>@endif
</div>
</div>
