{{--
  Hibarr branded meeting reminder — SMTP body (meeting created + .ics path).
  Structure matches deal-follow-up-reminder.plunk.html exactly.
  Subject: {{ mailSubject }} - {{ appName }} (see deal-follow-up-reminder.plunk.subject)
--}}
...
<span style="display:inline-block;width:18px;height:2px;background-color:#003160;vertical-align:middle;margin-right:10px"></span>{{ $badgeLabel }}
</td>
</tr>
</table>
</td>
...
<p>{{ $meetingMessage }}</p>{!! $meetingDetailHtml !!}{!! $joinMeetingHtml !!}{!! $crmButtonHtml !!}<p style="margin-top:20px">{{ $footerNote }}</p>
</td>
</tr>
<tr>
