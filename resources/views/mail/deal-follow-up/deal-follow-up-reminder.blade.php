{{--
  Hibarr branded meeting reminder — SMTP / UNS body fallback.
  Keep in sync with resources/views/mail/deal-follow-up/deal-follow-up-reminder.plunk.html
--}}
...
<span style="display:inline-block;width:18px;height:2px;background-color:#003160;vertical-align:middle;margin-right:10px"></span>Meeting Reminder
</td>
</tr>
</table>
</td>
...
@if($leadName !== '')
<div style="font-family:'DM Mono',Consolas,monospace;font-size:14px;color:#4a5272;line-height:1.6;margin-bottom:4px">{{ $leadName }}</div>
@endif
@if($meetingRemark !== '')
<div style="font-family:Syne,Georgia,serif;font-size:14px;line-height:22px;color:#0a0e1a;margin-top:14px">{!! nl2br(e($meetingRemark)) !!}</div>
@endif
</div>

@if($meetingLink !== '')
<p style="margin:20px 0 0;font-family:'DM Mono',Consolas,monospace;font-size:12px;line-height:1.7">
<a href="{{ $meetingLink }}" style="color:#003160;text-decoration:underline">Join meeting &rarr;</a>
</p>
@endif
