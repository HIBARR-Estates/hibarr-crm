...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addSubTaskPermission == 'added' && ($task->added_by == user()->id || $task->added_by == $userId || in_array($task->added_by, $clientIds)))
    || ($addSubTaskPermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addSubTaskPermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id || $task->added_by == $userId || in_array($task->added_by, $clientIds)))
    )
        <div class="p-20">
                            ...
                            <div class="d-flex">
                                ...
                                <div class="dropdown ml-auto subtask-action">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                        ...
                                        @if ($viewSubTaskPermission == 'all' || ($viewSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))))
                                            <a class="dropdown-item view-subtask" href="javascript:;"
                                               data-row-id="{{ $subtask->id }}">@lang('app.view')</a>
                                        @endif
                                        @if ($editSubTaskPermission == 'all' || ($editSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))) || ($editSubTaskPermission == 'owned' && $subtask->assigned_to == user()->id) || ($editSubTaskPermission == 'both' && ($subtask->assigned_to == user()->id || ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds)))))
                                            <a class="dropdown-item edit-subtask" href="javascript:;"
                                               data-row-id="{{ $subtask->id }}">@lang('app.edit')</a>
                                        @endif

                                        @if ($deleteSubTaskPermission == 'all' || ($deleteSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))) || ($deleteSubTaskPermission == 'owned' && $subtask->assigned_to == user()->id) || ($deleteSubTaskPermission == 'both' && ($subtask->assigned_to == user()->id || ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds)))))
                                            ...
                                            <x-slot name="action">
                                                <div class="dropdown ml-auto file-action">
                                                    ...
                                                    <div
                                                        ...
                                                        @if ($deleteSubTaskPermission == 'all' || ($deleteSubTaskPermission == 'added' && ($subtask->added_by == user()->id || $subtask->added_by === $userId || in_array($subtask->added_by, $clientIds))))
