...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskFilePermission == 'added' && ($task->added_by == user()->id || $task->added_by == $userId || in_array($file->added_by, $clientIds)))
    || ($addTaskFilePermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addTaskFilePermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id || $task->added_by == $userId || in_array($file->added_by, $clientIds)))
    )
        <div class="p-20">
