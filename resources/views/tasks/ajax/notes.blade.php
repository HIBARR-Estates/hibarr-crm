...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskNotePermission == 'added' && ($task->added_by == user()->id || $task->added_by == $userId || in_array($task->added_by, $clientIds)))
    || ($addTaskNotePermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addTaskNotePermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id || $task->added_by == $userId || in_array($task->added_by, $clientIds)))
    )
        ...
        @if ($task->approval_send == 1 && $isEmployee && !$isAdmin && $task->project->need_approval_by_admin == 1 && $status->slug == 'in_review')
