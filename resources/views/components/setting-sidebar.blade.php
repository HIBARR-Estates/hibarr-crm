...
<div class="settings-sidebar bg-white py-3" id="mob-settings-sidebar">
    ...
    <ul class="settings-menu" id="settingsMenu">
        ...
        @if (user()->permission('manage_company_setting') == 'all' && \App\Support\FeatureFlags::enabled('crm.entity-reminders'))
        ...
        @if (user()->permission('manage_lead_setting') == 'all' && in_array('leads', user_modules()))
            <x-setting-menu-item :active="$activeMenu" menu="meeting_types" :href="route('meeting-types.index')"
                                 :text="__('Meeting Types')"/>
        @endif
