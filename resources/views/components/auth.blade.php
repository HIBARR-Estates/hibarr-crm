...
<body
    ...
    <header class="px-4 bg-white sticky-top d-flex justify-content-center align-items-center login_header">
        ...
        @if (false)
            <h3
                class="mb-0 pl-1 {{ $globalSetting->auth_theme_text == 'light' ? ($globalSetting->auth_theme == 'dark' ? 'text-dark' : 'text-white') : '' }}">
                {{ $globalSetting->global_app_name ?? $globalSetting->app_name }}</h3>
        @endif
    </header>
                                ...
                                <div class="flex flex-wrap items-center justify-center gap-2">
                                    @foreach ($languages->take(4) as $index => $language)
                                        <span class="inline-flex">
                                            <a href="javascript:;" style="font-size: 12px !important;"
                                                class="text-dark-grey change-lang px-2 py-1 flex items-center"
                                                data-lang="{{ $language->language_code }}">
                                                <span style="height: 0px !important; line-height: 0px !important;"
