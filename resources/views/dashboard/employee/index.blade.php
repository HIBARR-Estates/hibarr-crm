...
@section('content')
    ...
    <div class="px-4 py-2 border-top-0 emp-dashboard">
        ...
        <div class="d-lg-flex d-md-flex d-block py-2 pb-4 align-items-center">
            ...
            <div class="ml-auto d-flex clock-in-out mb-3 mb-lg-0 mb-md-0 m mt-4 mt-lg-0 mt-md-0 justify-content-end">
                <p class="mb-0 text-lg-right text-md-right f-18 font-weight-bold text-dark-grey d-grid align-items-center">
                    ...
                    {{-- <span class="f-10 font-weight-normal">
                        {{ now()->timezone(company()->timezone)->translatedFormat('l') }}
                    </span> --}}
                    <span id="dashboard-clock" class="uppercase">
                ...
                @php
                    ...
                    $shiftAssigned = \App\Models\EmployeeShiftSchedule::where('user_id', $current_user_id)
                    ...
                    $isDefaultShiftAssignedToday = in_array(
                        $currentDayOfWeek,
                        json_decode($defaultShift->office_open_days, true),
                    );
                    ...
                    } elseif (!is_null($checkTodayHoliday) && $checkTodayHoliday->exists && !$shiftAssigned?->exists) {
                        $msg = __('messages.todayHoliday');
                    } elseif (
                        !is_null($checkTodayHoliday) &&
                        $checkTodayHoliday->exists &&
                        $shiftAssigned?->exists &&
                        $cannotLogin == true
                    ) {
                        $msg = __('messages.todayHoliday');
                    } elseif ($shiftAssigned == null && !$isDefaultShiftAssignedToday) {
                                        ...
                                        <div class="card-horizontal align-items-center">
                                            ...
                                            <div class="card-body border-0 pl-0">
                                                <div class="flex justify-between">
                                                    <h4 class="card-title text-dark f-18 f-w-500 mb-0">{{ $user->name }}
                                                    </h4>
