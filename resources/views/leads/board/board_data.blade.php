...
@foreach ($result['boardColumns'] as $key => $column)
    @if ($column->userSetting && $column->userSetting->collapsed)
        ...
        <div class="minimized bg-additional-grey rounded-xl mr-3">
            <!-- TASK BOARD HEADER START -->
            <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
                <a href="javascript:;" class="d-grid f-8 text-lightest mb-3 collapse-column"
                ...
                <p class="mb-3 mx-0 f-15 text-dark-grey font-weight-bold"><i class="fa fa-square mb-2 text-red"
                        style="color: {{ $column->label_color }}"></i>{{ $column->name }}
                </p>

                <span class="b-p-badge bg-grey f-13 px-2 py-2 font-weight-bold rounded d-inline-block"
    ...
    @else
        <!-- BOARD PANEL 2 START -->
        <div class="board-panel bg-additional-grey rounded-xl mr-3">
            <!-- TASK BOARD HEADER START -->
            <div class="mx-3 mt-3 mb-1 b-p-header">
                <div class="d-flex">
                    <p class="mb-0 f-15 mr-2 text-[#5C6068] font-normal text-truncate">
                        <i class="fa fa-square mr-2 text-yellow" style="color: {{ $column->label_color }}"></i>
                        {{ $column->name }}
                    </p>

                    <span class="b-p-badge bg-grey f-13 px-2 font-weight-bold rounded d-inline-block ml-1"
                        id="lead-column-count-{{ $column->id }}">{{ $column->deals_count }}</span>
                ...
                <div class="mr-3 mt-1 text-lg font-semibold">
            ...
            <div class="b-p-body">
                ...
                <div class="b-p-tasks" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}">
                    <div
                        class="card rounded bg-white border-grey m-1 mx-3 mb-3 no-task-card move-disable {{ count($column['deals']) > 0 ? 'd-none' : '' }}">
                        <div class="card-body">
