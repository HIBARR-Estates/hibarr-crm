...
interface MeetingFormFieldsProps {
    ...
    hostLocked?: boolean;
    /**
     * The record's already-resolved people (e.g. a follow-up's
     * `participant_users`, plus its host/creator) — passed straight through
     * to the host and participant pickers so an id already on the meeting
     * always shows its real name, even for someone outside the "employees"
     * directory those pickers search.
     */
    participantDirectory?: PersonOption[];
    /**
     * Which half to render, for a caller splitting the form across steps.
     *
     * "essentials" is just what the meeting is about — type and agenda, the
     * two things worth deciding before anything else. "details" is
     * everything about how it runs — when, the platform/link, host,
     * participants, reminders. Date/time/duration live in "details" rather
     * than "essentials" so picking a type doesn't get blocked on already
     * knowing the slot. Left undefined the whole form renders, which is what
...
export default function MeetingFormFields({
    ...
    const showEssentials = section !== "details";
    const showDetails = section !== "essentials";
        ...
        <div className="meeting-form-fields">
            {showEssentials && (
                <>
                    ...
                    <ModalField
                        label={t("pages.deals.workspace.meetings.agenda")}
                    >
                        <textarea
                            value={form.remark}
                            disabled={disabled}
            ...
            {showDetails && (
                <>
                    ...
                    {isVideoMeeting && (
                        <ModalField
                            label={td("Meeting provider", { source: "en" })}
                        >
                            ...
                            {form.meetingLink ? (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        />
                                        {t(
                                            "pages.deals.workspace.meetings.join",
                                        )}
                                    </a>
                                </div>
                            ) : (
                    ...
                    {hostFeatureEnabled && (
                        <ModalField
                            label={t(
                                "pages.deals.workspace.meetings.host_field",
                            )}
                        >
                            <HostField
                                value={form.hostId}
                                knownPeople={participantDirectory}
                                onChange={(hostId) => {
                                    // The host is tracked separately — never also a participant.
                                    const withoutNewHost =
                    ...
                    <ModalField
                        label={t(
                            "pages.deals.workspace.meetings.participants_field",
                        )}
                    >
                        <AssigneeField
                            value={
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        >
                                            {t(
                                                "pages.deals.workspace.meetings.before",
                                            )}
                                        </span>
                                        <Button
                                            variant="primary"
