...
// LeadWorkspaceContext, which orphan schedule surfaces have no business mounting.
...
/**
 * Meeting create for orphan/global schedule flows (Meetings index, Dashboard),
 * where the deal *or* lead is picked in the dialog itself. No workspace
 * context here (those only wrap the detail pages), so the caller reloads on
 * success instead of patching one.
 */
