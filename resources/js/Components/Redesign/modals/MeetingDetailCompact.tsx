...
import { meetingRecordLink } from "@/Components/Redesign/meeting/meetingRecordLink";
...
interface MeetingDetailCompactProps {
    ...
    canDelete: boolean;
    /**
     * Rescheduling separately from editing. Defaults to canEdit.
     * Dashboard / next-action surfaces need them apart (edit needs a deal;
     * reschedule does not).
     */
    canReschedule?: boolean;
    /** Cancelling separately from editing. Defaults to canEdit. */
    canCancel?: boolean;
    userId?: number;
    onClose: () => void;
    onEdit: () => void;
    onDelete: () => void;
    /** Opens the caller's reschedule form (nested or sibling). */
    onReschedule?: () => void;
    onCancelMeeting?: () => void;
    onMarkHeld?: () => void;
    isUpdating?: boolean;
    /**
     * The AI summary, shown under the second tab. Omit to hide the Summary tab
     * (dashboard / next-action callers that do not ship summary content).
     */
...
export default function MeetingDetailCompact({
    ...
    const [panel, setPanel] = useState<Panel>(
        initialPanel === "summary" && summaryPanel ? "summary" : "info",
    );
    ...
    return (
        <>
            ...
            <ConfirmDialog
                open={confirmCancel}
                title={td("Cancel this meeting?")}
                message={td(
                    "The meeting is marked cancelled. Attendees are notified.",
                )}
                confirmLabel={t(
                    "pages.deals.workspace.meetings.cancel_meeting",
                )}
