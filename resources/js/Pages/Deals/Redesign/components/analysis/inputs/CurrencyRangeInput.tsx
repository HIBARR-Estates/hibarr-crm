...
function parse(value: string): { code: string; min: string; max: string } {
    const [code = "GBP", min = "", max = ""] = value.split("|");
    return { code, min, max };
}
...
export default function CurrencyRangeInput({ value, onChange }: CurrencyRangeInputProps) {
    ...
    const update = (newCode: string, newMin: string, newMax: string) =>
                        ...
                        <div className="max-h-48 overflow-y-auto py-1">
                            {filtered.map((c) => (
                                <button
                                    key={c.code}
                                    type="button"
                                    onClick={() => { update(c.code, min, max); setOpen(false); setSearch(""); }}
                                ...
                                >
                                    ...
                                    <span className="text-xs" style={{ opacity: c.code === code ? 0.7 : 1, color: c.code === code ? "#fff" : T.TEXT_MUTED }}>
                                        {c.name}
                                    </span>
                                </button>
                            ))}
                ...
                <div className="flex-1">
                    ...
                    <div className="flex">
                        ...
                        <input
                            ...
                            value={fmt(min)}
                            placeholder="0"
                            onChange={(e) => update(code, e.target.value.replace(/[^0-9]/g, ""), max)}
                            className={fieldClass}
                            style={{ color: T.TEXT }}
                        />
                    </div>
                </div>
                <span className="mt-5" style={{ color: T.BORDER }}>—</span>
                <div className="flex-1">
                    <label
                        className="block text-[10px] font-semibold uppercase tracking-wider mb-1"
                        style={{ color: T.TEXT_MUTED }}
                    ...
                    <div className="flex">
                        ...
                        <input
                            ...
                            value={fmt(max)}
                            placeholder="0"
                            onChange={(e) => update(code, min, e.target.value.replace(/[^0-9]/g, ""))}
                            className={fieldClass}
                            style={{ color: T.TEXT }}
                        />
                    </div>
