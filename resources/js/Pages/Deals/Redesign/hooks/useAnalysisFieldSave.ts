...
export default function useAnalysisFieldSave(dealId: number) {
    ...
    const isCustomFieldKey = useCallback(
        (key: string) => key.startsWith("deal_field_") || key.startsWith("lead_field_"),
        [],
    );

    const toCustomFieldEntry = useCallback((key: string, value: unknown): CustomFieldEntry => {
        if (key.startsWith("deal_field_")) {
            return { scope: "deal", fieldKey: `field_${key.replace("deal_field_", "")}`, value };
        }
        return { scope: "lead", fieldKey: `field_${key.replace("lead_field_", "")}`, value };
    }, []);
        ...
        (entries: Map<string, CustomFieldEntry>) => {
            ...
            return track(
                axios
                    ...
                    .catch(() => {
                        setFailedKeys((prev) => {
                            const filtered = prev.filter((f) => !keys.includes(f.key));
                            const retried = keys.map((key) => ({
                                key,
                                retry: () => {
                                    const entry = entries.get(key);
                                    if (!entry) return;
                                    setFailedKeys((p) => p.filter((f) => f.key !== key));
    ...
    const sendLegacy = useCallback(
        (key: string, payload: { type: string; data: Record<string, unknown> }) => {
            return track(
                axios
                    .patch(
                        route("deals.gathering.inline_update", { id: dealId }),
    ...
    const save = useCallback(
        (key: string, value: unknown) => {
            if (bulkWriteEnabled && isCustomFieldKey(key)) {
                customFieldPending.current.set(key, toCustomFieldEntry(key, value));
                if (customFieldTimer.current) clearTimeout(customFieldTimer.current);
        ...
        [bulkWriteEnabled, isCustomFieldKey, toCustomFieldEntry, flushCustomFields, notify, saveLegacy],
    );
