...
function eventToRow(event: CrmEvent): TimelineEventRow {
    ...
    const actor =
        event.user?.name ||
        (type === "system" ? "System" : type === "external" ? "External" : "Agent");
    ...
    return {
        ...
        grouped:
            grouped ||
            (details && details.length > 0 ? `${details.length} fields changed` : undefined),
        details: details && details.length > 0 ? details : undefined,
        preview: previewSource ? `"${previewSource}"` : undefined,
    };
}

export default function ActivityTab({ leadId }: ActivityTabProps) {
    ...
    const filteredEvents = useMemo(() => (data?.data ?? []).map(eventToRow), [data]);
                ...
                {(["all", "agent", "system", "external"] as TimelineFilter[]).map(
                    (currentFilter) => (
                        <button
                            ...
                            style={{
                                background:
                                    filter === currentFilter ? T.NAVY : T.WHITE,
                                color:
                                    filter === currentFilter ? T.WHITE : T.TEXT_MUTED,
                                borderColor:
                                    filter === currentFilter ? T.NAVY : T.BORDER,
                    ...
                    >
                        <div className="flex flex-col items-center pt-1">
                            <div
                                ...
                                style={{
                                    background:
                                        event.dot === "agent" ? T.GREEN : T.GRAY_MID,
                                }}
                            />
                        ...
                        <div className="flex-1 pb-1">
                            ...
                            <div className="text-[11px] text-[#9ca3af]">{event.meta}</div>
                            {event.grouped && (
                                <div>
                                    <button
                                        type="button"
