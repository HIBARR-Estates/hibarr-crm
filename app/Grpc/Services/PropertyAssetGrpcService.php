...
use App\Grpc\Generated\PropertyAsset\PropertyAsset as PropertyAssetMessage;
...
class PropertyAssetGrpcService implements PropertyAssetServiceInterface
{
    public function List(ContextInterface $ctx, ListPropertyAssetsRequest $in): ListPropertyAssetsResponse
    {
        try {
            ...
            $query = PropertyAsset::where('company_id', $companyId);

            if ($in->hasPropertyId()) {
                $query->where('property_id', $in->getPropertyId());
            }
            if ($in->hasAssetType()) {
            ...
            $allowedSortColumns = ['id', 'property_id', 'name', 'asset_type', 'order', 'created_at', 'updated_at'];
            ...
            $response = new ListPropertyAssetsResponse();
    ...
    private function buildMessage(PropertyAsset $record): PropertyAssetMessage
    {
        $msg = new PropertyAssetMessage();
        $msg->setId($record->id);
        $msg->setPropertyId($record->property_id ?? 0);
    ...
    {
        \Log::error('PropertyAssetGrpcService error: ' . $e->getMessage(), ['exception' => $e]);
