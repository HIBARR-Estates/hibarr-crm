<?php

namespace App\Grpc\Services;

use App\Grpc\Interfaces\TaskboardColumnServiceInterface;
use App\Grpc\Generated\TaskboardColumn\ListTaskboardColumnsRequest;
use App\Grpc\Generated\TaskboardColumn\ListTaskboardColumnsResponse;
use App\Grpc\Generated\TaskboardColumn\StreamTaskboardColumnsRequest;
use App\Grpc\Generated\TaskboardColumn\TaskboardColumnBatch;
use App\Grpc\Generated\TaskboardColumn\TaskboardColumn as TaskboardColumnMessage;
use App\Grpc\Generated\Common\PaginationMeta;
use App\Grpc\Generated\Common\StreamProgress;
use App\Models\TaskboardColumn;
use Spiral\RoadRunner\GRPC\ContextInterface;
use Spiral\RoadRunner\GRPC\Exception\GRPCException;
use Spiral\RoadRunner\GRPC\StatusCode;

class TaskboardColumnGrpcService implements TaskboardColumnServiceInterface
{
    public function List(ContextInterface $ctx, ListTaskboardColumnsRequest $in): ListTaskboardColumnsResponse
    {
        try {
            $companyId = $this->getCompanyId($ctx);

            $pagination = $in->getPagination();
            $page = max(1, $pagination ? $pagination->getPage() : 1);
            $perPage = min(100, max(1, $pagination ? $pagination->getPerPage() : 15));
            $sortBy = $pagination ? $pagination->getSortBy() : 'priority';
            $sortOrder = $pagination && strtolower($pagination->getSortOrder()) === 'desc' ? 'desc' : 'asc';

            $query = TaskboardColumn::where('company_id', $companyId);

            if ($in->hasSearch()) {
                $search = $in->getSearch();
                $query->where('column_name', 'like', "%{$search}%");
            }

            $allowedSortColumns = ['id', 'column_name', 'priority', 'created_at', 'updated_at'];
            if (!in_array($sortBy, $allowedSortColumns)) {
                $sortBy = 'priority';
            }

            $paginator = $query->orderBy($sortBy, $sortOrder)->paginate($perPage, ['*'], 'page', $page);

            $response = new ListTaskboardColumnsResponse();
            foreach ($paginator->items() as $column) {
                $response->getResult()[] = $this->buildMessage($column);
            }

            $meta = new PaginationMeta();
            $meta->setCurrentPage($paginator->currentPage());
            $meta->setPerPage($paginator->perPage());
            $meta->setTotal($paginator->total());
            $meta->setTotalPages($paginator->lastPage());
            $meta->setHasMore($paginator->hasMorePages());
            $response->setPagination($meta);

            return $response;
        } catch (GRPCException $e) {
            throw $e;
        } catch (\Throwable $e) {
            throw $this->mapException($e);
        }
    }

    // ── Stream (bulk dump) ─────────────────────────────────

    public function Stream(ContextInterface $ctx, StreamTaskboardColumnsRequest $in): TaskboardColumnBatch
    {
        try {
            $companyId = $this->getCompanyId($ctx);

            $streamParams = $in->getStreamParams();
            $sinceId = $streamParams ? $streamParams->getSinceId() : 0;
            $sinceUpdated = $streamParams ? $streamParams->getSinceUpdated() : '';
            $ids = $streamParams ? iterator_to_array($streamParams->getIds()) : [];

            $query = TaskboardColumn::where('company_id', $companyId);

            if ($sinceId > 0) {
                $query->where('id', '>', $sinceId);
            }
            if (!empty($sinceUpdated)) {
                $query->where('updated_at', '>', $sinceUpdated);
            }
            if (!empty($ids)) {
                $query->whereIn('id', $ids);
            }

            if ($in->hasSearch()) {
                $search = $in->getSearch();
                $query->where('column_name', 'like', "%{$search}%");
            }

            $total = $query->count();
            $records = $query->orderBy('id', 'asc')->get();

            $batch = new TaskboardColumnBatch();
            foreach ($records as $column) {
                $batch->getResult()[] = $this->buildMessage($column);
            }

            $progress = new StreamProgress();
            $progress->setProcessed($records->count());
            $progress->setTotal($total);
            $progress->setIsComplete(true);
            $batch->setProgress($progress);

            return $batch;
        } catch (GRPCException $e) {
            throw $e;
        } catch (\Throwable $e) {
            throw $this->mapException($e);
        }
    }

    // ── Helpers ────────────────────────────────────────────

    private function buildMessage(TaskboardColumn $column): TaskboardColumnMessage
    {
        $msg = new TaskboardColumnMessage();
        $msg->setId($column->id);
        $msg->setCompanyId($column->company_id ?? 0);
        $msg->setColumnName($column->column_name ?? '');
        $msg->setSlug($column->slug ?? '');
        $msg->setLabelColor($column->label_color ?? '');
        $msg->setPriority($column->priority ?? 0);
        $msg->setCreatedAt($this->dateToString($column->created_at));
        $msg->setUpdatedAt($this->dateToString($column->updated_at));

        return $msg;
    }

    private function getCompanyId(ContextInterface $ctx): int
    {
        $companyId = $ctx->getValue('authenticated_company_id');
        if (!$companyId) {
            throw new GRPCException(
                'Authentication required. Provide x-api-token and x-company-id in gRPC metadata.',
                StatusCode::UNAUTHENTICATED,
            );
        }
        return (int) $companyId;
    }

    private function dateToString(mixed $value): string
    {
        if ($value === null) return '';
        if ($value instanceof \DateTimeInterface) return $value->format('c');
        return (string) $value;
    }

    private function mapException(\Throwable $e): GRPCException
    {
        \Log::error('TaskboardColumnGrpcService error: ' . $e->getMessage(), ['exception' => $e]);
        return new GRPCException('Internal error: ' . $e->getMessage(), StatusCode::INTERNAL);
    }
}
