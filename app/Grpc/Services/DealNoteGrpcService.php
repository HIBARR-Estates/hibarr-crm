...
use App\Grpc\Generated\DealNote\DealNote as DealNoteMessage;
...
class DealNoteGrpcService implements DealNoteServiceInterface
{
    public function List(ContextInterface $ctx, ListDealNotesRequest $in): ListDealNotesResponse
    {
        try {
            ...
            $query = DealNote::whereIn('deal_id', Deal::where('company_id', $companyId)->select('id'));

            if ($in->hasDealId()) {
                $query->where('deal_id', $in->getDealId());
            }
            if ($in->hasAddedBy()) {
                $query->where('added_by', $in->getAddedBy());
            }
            if ($in->hasSearch()) {
                $search = $in->getSearch();
            ...
            $allowedSortColumns = ['id', 'deal_id', 'title', 'created_at', 'updated_at'];
            ...
            $response = new ListDealNotesResponse();
            foreach ($paginator->items() as $note) {
                $response->getResult()[] = $this->buildMessage($note);
            }
    ...
    public function Stream(ContextInterface $ctx, StreamDealNotesRequest $in): DealNoteBatch
    {
        try {
            ...
            $batch = new DealNoteBatch();
            foreach ($records as $note) {
                $batch->getResult()[] = $this->buildMessage($note);
            }
    ...
    private function buildMessage(DealNote $note): DealNoteMessage
    {
        $msg = new DealNoteMessage();
        $msg->setId($note->id);
        $msg->setDealId($note->deal_id ?? 0);
        $msg->setAddedBy($note->added_by ?? 0);
        $msg->setLastUpdatedBy($note->last_updated_by ?? 0);
        $msg->setTitle($note->title ?? '');
        $msg->setDetails($note->details ?? '');
        $msg->setCreatedAt($this->dateToString($note->created_at));
        $msg->setUpdatedAt($this->dateToString($note->updated_at));
    ...
    {
        \Log::error('DealNoteGrpcService error: ' . $e->getMessage(), ['exception' => $e]);
