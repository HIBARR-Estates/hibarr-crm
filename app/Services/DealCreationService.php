    ...
    public function processDeal(int $contactId, int $companyId, array $requestData): array
    {
        ...
        try {
            ...
            DB::afterCommit(function () use ($cacheKey, $contactLockKey, &$deal, $isNewDeal, $request, $companyId, $packageExplicitlySelected) {
                ...
                // Fired last so a failure in any of the non-critical operations
                // above (meeting creation, offers, ...) still lets the
                // automation run — not from inside the transaction earlier,
                // where none of that had happened yet. saveQuietly() never
                // fires DealObserver, so deal_created/deal_updated never see
                // API-originated deals at all; deal_created_api/deal_updated_api
                // are the only way an automation can react to this write. A
                // fresh reload guards against any of the above having mutated
                // the row without also updating this in-memory $deal.
                $runStep('api_automation', function () use ($deal, $isNewDeal): void {
